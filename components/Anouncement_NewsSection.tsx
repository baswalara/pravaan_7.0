...
const Anouncement_NewsSection = () => {
  return (
    <>
      <div
        className={`bg-repeat bg-center  w-full text-black`}
        style={{ backgroundImage: "url('/assets/anouncement/nbg.avif')" }}
      >
        <div className="flex flex-col justify-center items-center">
          <div
            className={`pt-[6rem] max-sm:pt-[4rem] text-[70px] ${aboutNPfont.className}`}
          >
            The Exodia Times
          </div>
          ...
          <div
            className={`flex items-center justify-between max-sm:justify-center mt-2 mb-2 w-[80%] text-[20px] ${aboutNPfont2.className}`}
          >
            <div className="max-sm:hidden"></div>
            <div className="max-sm:hidden">Prayaan</div>
          ...
          <div className="flex flex-col justify-center items-center w-[80%]  max-sm:w-[90%] mt-7 max-sm:mt-8">
            {announcements.map((item, index) => (
              <>
                ...
                >
                  <div
                    className={`w-[75%]  left-0 flex flex-col justify-center items-center`}
                  >
                    <div
                      className={`w-[80%] ${aboutNPfont3.className} font-serif italic text-[40px] max-sm:text-[20px] text-start`}
                    >
                      {item.title}
                    </div>
                    <div className="flex max-sm:flex-col w-[80%] mt-4 mb-8 gap-12 max-sm:gap-0">
                      {item.image && (
                        <>
                          <div className="w-[40%] flex justify-start items-center ">
                            <Image
                              className="w-[20rem] border border-black transition-all duration-500 hover:grayscale-0 grayscale"
                              src={item.image}
                              alt={item.title}
                              width={200}
                              height={200}
                          ...
                          <div
                            className={`${item.image ? "w-[30%] max-sm:w-[100%] mt-2" : "w-[80%]"} flex items-start`}
                          >
                            {/* Combined Description with Drop Cap */}
                            <p
                              className={`text-[15px] max-sm:text-[10px] ${aboutNPfont4.className} text-start`}
                            >
                              <span
                      ...
                      {!item.image && (
                        <div
                          className={`${item.image ? "w-[30%] max-sm:w-[100%] mt-2" : "w-[80%]"} text-[15px] max-sm:text-[10px] ${aboutNPfont4.className} text-start `}
                        >
                          {item.description}
                        </div>
                      )}
                  ...
                  <div className="w-[25%] max-sm:w-[15%] right-0 flex flex-col justify-center items-center">
                    <div
                      className={`${aboutNPfont4.className} max-sm:text-[10px]`}
                    >
                      {item.day}
                    </div>
                    <div
